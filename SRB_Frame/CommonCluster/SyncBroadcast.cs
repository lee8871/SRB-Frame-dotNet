    ...
    public partial class BaseNode
    {
        public partial class SyncCluster : BaseNode.ICluster
        {
            public class Broadcast
            {
                ...
                IBus bus;
                Stopwatch sync_clocks;
                public Broadcast(IBus bus)
                {
                    sync_clocks = new Stopwatch();
                    sync_clocks.Restart();
                    this.bus = bus;
                    Sync_public_sno = (byte)support.random.Next(127);
                }

                public int getSrbClock()
                {
                    return getSrbClock(sync_clock.ElapsedTicks);
                }
                public int getSrbClock(long elapsed_ticks)
                {
                    return (int)(((long)Math.Round(tickToClock(elapsed_ticks))) & 0xffffff);
                }
                public long getTotolTicks()
                {
                    return getTotolTicks(sync_clock.ElapsedTicks);
                }
                public long getTotolTicks(long elapsed_ticks)
                {
                    return ((long)Math.Round(tickToClock(elapsed_ticks))) >> 8;
                }
